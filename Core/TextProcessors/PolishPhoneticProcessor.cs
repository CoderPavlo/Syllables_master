...
namespace Sklady.TextProcessors
{
    public class PolishPhoneticProcessor : PhoneticProcessorBase
    {
        ...
        {
            //var res = ProcessNonPolishWords(input);
        ...
        private string ProcessNonPolishWords(string word)
        {
            string pattern = @"^[a-pr-zA-PR-ZĄĆĘŁŃÓŚŻŹąćęłńóśżź]+$";
            if (Regex.IsMatch(word, pattern))
                return word;

            return Regex.IsMatch(word, "-") ? string.Empty : null;
        }
